/*
  ...
  2. Profile Enhancements
    - Add `long_term_persona` field for deep psychological profiles
    - Add `persona_updated_at` timestamp
  ...
  5. Automation
    - Cron jobs for weekly and monthly summary generation
*/
...
-- 8. Set up cron jobs for automated summary generation (with existence checks)
DO $$
BEGIN
  -- Check if weekly cron job already exists, if not create it
  IF NOT EXISTS (
    SELECT 1 FROM cron.job WHERE jobname = 'generate-weekly-summaries'
  ) THEN
  ...
  -- Check if monthly cron job already exists, if not create it
  IF NOT EXISTS (
    SELECT 1 FROM cron.job WHERE jobname = 'generate-monthly-summaries'
  ) THEN
    PERFORM cron.schedule(

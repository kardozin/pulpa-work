/*
# Periodic Summaries and Enhanced Analytics System

This migration adds comprehensive analytics capabilities to the pulpa.work platform:

1. New Tables
   - `periodic_summaries` - Stores automated weekly/monthly analysis
   
2. Enhanced Profiles
   - `long_term_persona` - AI-generated psychological profile
   - `persona_updated_at` - Timestamp for persona updates
...
4. Functions
   - `insert_periodic_summary` - Atomic summary insertion with conflict handling
   - `get_user_context_for_ai` - Enhanced context retrieval for AI conversations
   
5. Automation
   - Cron jobs for weekly (Mondays 02:00 UTC) and monthly (1st of month 03:00 UTC) summary generation
...
-- 3. Enable RLS on periodic_summaries
ALTER TABLE periodic_summaries ENABLE ROW LEVEL SECURITY;

-- 4. Security policies for periodic_summaries
...
-- 8. Set up cron jobs for automated summary generation
